...
data "terraform_remote_state" "db" {
  backend = "local"
  config = {
    path = "${path.module}/../../db-service-1/dev/terraform.tfstate"
  }
...
# ── App subnet -> NAT route table associations ────────────────────────────────
# Web service decides which private subnets are "app tier" and wires them to NAT.
